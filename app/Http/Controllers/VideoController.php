...
class VideoController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('welcome_page.video.index');
    }
    ...
    public function edit($id)
    {
        ...
        return view('welcome_page.video.edit', compact('video'));
    }
    ...
    public function getVideo(Request $request)
    {
        ...
        $query = Video::query()->select('*');
        //dd($query->toSql());
        if ($request->has('search') && !empty($request->search)) {
            $search = strtolower($request->search); // Konversi ke huruf kecil untuk pencarian tidak case-sensitive
            $query->where(function ($q) use ($search) {
                $q->whereRaw('LOWER(video.judul) LIKE ?', ["%{$search}%"])
                    ->orWhereRaw('LOWER(video.isi_video) LIKE ?', ["%{$search}%"]);
            });
        }

        if ($sort === 'oldest') {
            $query->orderBy('video.tgl_posting', 'ASC');
        } elseif ($sort === 'newest') {
            $query->orderBy('video.tgl_posting', 'DESC');
        } else {
            $query->orderBy('video.tgl_posting', 'DESC');
        }
