				...
				<div class="container">
                    ...
                    <div class="d-flex align-items-center justify-content-between">
                        {{-- <a href="{{ route('catalog.create') }}" class="btn btn-primary">Tambah</a> --}}
                         <input type="text" id="search-input" class="form-control" placeholder="Cari berita..." style="max-width: 300px;">
                    </div>
...
function loadProducts(sort = 'newest', page = 1, search = '') {
    const apiUrl = `{{ url('/api/video') }}?sort=${sort}&page=${page}&search=${encodeURIComponent(search)}`;
    console.log(apiUrl);
        ...
        .then(data => {
            ...
            if (data.data.length > 0) {
                data.data.forEach(catalog => {
                    const assetPath = "https://ptgib.co.id/asset/img_video/";
                    const urlPath = "{{ url('/video/') }}";
                                ...
                                <div class="card-body">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="card-side-img d-flex">
                                            <!-- Gambar Utama -->
                                            ${catalog.youtube ? `
                                                <a class="product-thumbnail d-block me-2" href="${urlPath}/${catalog.id_video}/edit">
                                                    <div style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden;">
                                                        <iframe width="150" height="90" src="${catalog.youtube}" frameborder="0" allowfullscreen></iframe>
                                                    </div>
                                                </a>
                                            ` : ''}
                                        ...
                                        <div class="card-content px-4 py-2">
                                            <a style="font-size:13px" href="${urlPath}/${catalog.id_video}/edit">
                                                ${catalog.judul}
