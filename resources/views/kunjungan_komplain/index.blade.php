...
function loadProducts(sort = 'newest', page = 1, search = '') {
    const apiUrl = `{{ url('/api/kunjungan_komplain') }}?sort=${sort}&page=${page}&search=${encodeURIComponent(search)}`;
    console.log(apiUrl);
        ...
        .then(data => {
            ...
            if (data.data.length > 0) {
                data.data.forEach(kunjungan => {
                    ...
                    const urlPath = "{{ url('/kunjungan_komplain/') }}";
                                    ...
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="card-content px-4 py-2">
                                            <a class="product-title d-block text-truncate mt-0" href="${urlPath}/${kunjungan.id}/edit">
                                                ...
                                                ${kunjungan.alamat}
                                                <br/>
                                                ${formatTanggal(kunjungan.tgl_kunjungan)} ${kunjungan.jam} WIB
                                                <br/>
                                               Catatan : ${kunjungan.pesan ? kunjungan.pesan.substring(0, 50) + (kunjungan.pesan.length > 50 ? '...' : '') : ''}
                                            </a>
                                            ...
                                            ` : ''}
                                            <br/>
                                            <a type="button" class="btn-sm btn-warning" href="${urlPath}/create?id_kunjungan=${kunjungan.id}&id_unit=${kunjungan.id_unit}&id_lokasi=${kunjungan.id_lokasi}&id_customer=${kunjungan.id_customer}">
                                                 ${kunjungan.laporan_komplain ? `Ubah Laporan`:`Buat Laporan`}
                                            </a>
